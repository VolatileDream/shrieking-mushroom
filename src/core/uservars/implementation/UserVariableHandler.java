...
import core.uservars.exceptions.VariableNotBoolException;
import core.uservars.exceptions.VariableNotIntException;
...
public class UserVariableHandler implements IVariableHandler {
	...
	public int GetRequiredVariableAsInt( String name, IVariableStore store ){
		...
		throwRequiredVariableMissingError(name, new VariableNotIntException(name) );
	...
	public boolean GetRequiredVariableAsBoolean( String name, IVariableStore store ) {
		...
		throwRequiredVariableMissingError(name, new VariableNotBoolException( name ) );
	...
	private void throwRequiredVariableMissingError( String name, Exception byProduct ) throws VariableError {
	...
	private String getRequiredVariableMissingMessage( String name ){
		return "Variable : '" +name+ "' couldn't be found in the existing or default variable stores";
	}
}
